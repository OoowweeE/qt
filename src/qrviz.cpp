...
// *通过传入QVBoxLayout，建立qrviz和main_window这两个页面的链接*
...
void qrviz::ModifyTFAttribute(rviz::Display* TF, bool Enabled)
{
  ...
  TF->subProp("Reference Frame")->setValue("<Fixed Frame>");
  //TF->subProp("Reference Frame")->setValue("<Fixed Frame>");
//  TF->subProp("Target Frame")->setValue("base_link");  // 目标框架，例如机器人坐标系
////  TF->subProp("Reference Frame")->setValue("<Fixed Frame>");
//  //TF->subProp("Target Frame")->setValue("base_link");  // 或者是动态设置的目标框架名
//  TF->subProp("Update Interval")->setValue(0.1);  // 设置更新频率为 0.1 秒
//  TF->subProp("Time Display")->setValue(true);  // 启用时间戳显示
//  TF->subProp("Arrow Size")->setValue(0.2);  // 设置箭头大小
//  TF->subProp("Color")->setValue(QColor(255,33,255));   // 设置颜色为红色
//  TF->subProp("Alpha")->setValue(1.0);       // 设置透明度为完全不透明
  TF->subProp("Length")->setValue(1.0);       // 设置透明度为完全不透明
  TF->subProp("Radius")->setValue(0.1);       // 设置透明度为完全不透明
}
...
/**
 * @brief 修改标记数组Display对象属性方法
 * @param rviz::Display* Image        ———— 图像Display对象
 * @param QString ImageTopic          ———— 标记图像话题字符串
 * @param bool Enabled                ———— 是否启用Display对象
 */
void qrviz::ModifyImageAttribute(rviz::Display *Image, QString ImageTopic, bool Enabled)
{
    Image->setEnabled(Enabled);//启用image
    Image->subProp("Image Topic")->setValue(ImageTopic);//修改图像绑定的话题
    Image->subProp("Transport Hint")->setValue("raw");
    Image->subProp("Queue size")->setValue(2);
}

void qrviz::ModifyPoseAttribute(rviz::Display *Pose, QString PoseTopic, QColor PoseColor, bool Enabled)
{
    Pose->setEnabled(Enabled);//启用image
    Pose->subProp("Topic")->setValue(PoseTopic);//修改图像绑定的话题
}

void qrviz::ModifyBoundingBoxArrayAttribute(rviz::Display *BoundingBoxArray, QString BoundingBoxArrayTopic, bool Enabled)
{
    BoundingBoxArray->setEnabled(Enabled);//启用image
    BoundingBoxArray->subProp("Topic")->setValue(BoundingBoxArrayTopic);//修改图像绑定的话题
    //BoundingBoxArray->subProp("Coloring")->setValue("Auto");
    //BoundingBoxArray->subProp("Coloring")->setValue("Value");
}
