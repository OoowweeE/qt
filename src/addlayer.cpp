  ...
  ui(new Ui::AddLayer)
{
  ...
  markerArrayTopicList = QStringList();
  imageTopicList = QStringList();
  poseTopicList = QStringList();
  BoundingBoxArrayTopicList = QStringList();//这些静态成员变量比较特殊，本来静态成员变量是属于类的所以不要在构造函数里初始化它们，但这里我们需要在构造函数里初始化它们。因为它们本不应该被做成静态变量的，只是我想让其访问方便
  // 001-初始化一组静态成员变量，用于存储图层和话题信息。

  // 002-基于QTreeWidgetItem构建树形界面UI（不含逻辑）
  /*********************
  ...
  /*********************/
  _image = new QTreeWidgetItem(QStringList() << "图像");
  _image->setIcon(0, QIcon("://rViz/Image.png"));
  _rviz->addChild(_image);                              //添加image子节点
  /*********************/
  _pose = new QTreeWidgetItem(QStringList() << "pose");
  _pose->setIcon(0, QIcon("://rViz/Pose.png"));
  _rviz->addChild(_pose);                              //添加pose子节点
  /*********************/
  _boundingboxarray = new QTreeWidgetItem(QStringList() << "boundingboxarray");
  _boundingboxarray->setIcon(0, QIcon("://rViz/BoundingBoxArray.png"));
  _rviz->addChild(_boundingboxarray);                              //添加boundingboxarray子节点
  /*********************/
  // 002-基于QTreeWidgetItem构建树形界面UI（不含逻辑）
  ...
  /*********************
   *  connect 操作：将UI中的按钮和树形控件的信号与槽函数连接，用于处理用户交互事件。
   *********************/
...
QStringList AddLayer::imageTopicList = QStringList();
QStringList AddLayer::poseTopicList = QStringList();
QStringList AddLayer::BoundingBoxArrayTopicList = QStringList();

/**
 * @brief treeWidget_AllLayer“当前项改变”信号处理槽函数（点击不同的项，对应显示不同的注释）
 */
void AddLayer::on_treeWidget_AllLayer_currentItemChanged()
{
  ...
  else
  {
    ...
    switch(ui->treeWidget_AllLayer->topLevelItem(0)->indexOfChild(ui->treeWidget_AllLayer->currentItem()))
    {
      ...
      break;
      case 10:
        ui->textBrowser_Description->setText("图像\n\n\tDisplays an image from a sensor_msgs/Image topic, similar to image_view. ");
        ui->lineEdit_DisplayName->setText("图像");
      break;
      case 11:
        ui->textBrowser_Description->setText("Pose\n\n\tDisplays a geometry_msgs::PoseStamped message.  ");
        ui->lineEdit_DisplayName->setText("Pose");
      break;
      case 12:
        ui->textBrowser_Description->setText("boundingboxarray\n\n\tvisualize jsk_recognition_msgs/BoundingBoxArray  ");
        ui->lineEdit_DisplayName->setText("BoundingBoxArray");
      break;
    }
  }
}
...
/**
 * @brief treeWidget_ByTopic“当前项改变”信号处理槽函数（点击不同的项，对应显示不同的注释）
 */
...
void AddLayer::treeWidget_ByTopic_addItemByTopic(QTreeWidget *treeWidget, std::string name, std::string datatype)
{
  ...
  if(MsgTypeHash.contains(msgType)){
    ...
    switch (MsgTypeHash.value(msgType)) {
      ...
      break;
      case AddLayer::image:
        lastItem->setText(0, "image");
        lastItem->setIcon(0, QIcon("://rViz/Image.png"));
      break;
      case AddLayer::pose:
        lastItem->setText(0, "pose");
        lastItem->setIcon(0, QIcon("://rViz/Pose.png"));
      break;
      case AddLayer::boundingboxarray:
        lastItem->setText(0, "boundingboxarray");
        lastItem->setIcon(0, QIcon("://rViz/BoundingBoxArray.png"));
      break;
    }
 ...
 * 据当前选择的 tab 和 item 设置 LayerCodeToReturn、DisplayName 和 TopicToSubscribe
 */
void AddLayer::slot_Accepted()
{
  if(ui->tabWidget->currentWidget() == ui->tab_ByDisplayType)
  {
    switch(ui->treeWidget_AllLayer->topLevelItem(0)->indexOfChild(ui->treeWidget_AllLayer->currentItem()))
    {
      ...
      break;
      case 10:
        LayerCodeToReturn = AddLayer::image;
      break;
      case 11:
        LayerCodeToReturn = AddLayer::pose;
      break;
      case 12:
        LayerCodeToReturn = AddLayer::boundingboxarray;
      break;
    }
  }
...
int AddLayer::getLayerInfo(QWidget *parent)
{
    AddLayer addLayer(parent);
    addLayer.close();
    return LayerCodeToReturn;
...
void AddLayer::topicListAppendByTopicInfo(const ros::master::TopicInfo& info)
{
  switch (MsgTypeHash.value(QString::fromStdString(info.datatype))) {
    ...
    break;
    case AddLayer::image:
     imageTopicList.append(QString::fromStdString(info.name));
    break;
    case AddLayer::pose:
     imageTopicList.append(QString::fromStdString(info.name));
    break;
    case AddLayer::boundingboxarray:
      BoundingBoxArrayTopicList.append(QString::fromStdString(info.name));
    break;
  }
}
